            ...
            <div className="container mx-auto px-4 md:px-6 py-12">
                <div className="mb-12">
                    <nav className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-slate-400 mb-4">
                        <Link href="/cart" className="hover:text-emerald-600 transition-colors">ঝুলি</Link>
                        <ChevronRight size={12} />
                        <span className="text-emerald-600">চেকআউট</span>
                    </nav>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col lg:grid lg:grid-cols-12 gap-16">
                    ...
                    <div className="lg:col-span-7 space-y-12">
                        <section>
                            <div className="flex items-center gap-4 mb-8">
                                ...
                                <h2 className="text-2xl font-black text-slate-800 tracking-tight">ডেলিভারির ঠিকানা</h2>
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 px-1">আপনার পুরো নাম</label>
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            className="w-full bg-white border border-slate-100 pl-14 pr-6 py-4 rounded-2xl text-sm font-bold focus:ring-2 focus:ring-emerald-100 transition-all shadow-sm"
                                        />
                                    </div>
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 px-1">মোবাইল নম্বর</label>
                                    <div className="relative">
                                ...
                                <div className="md:col-span-2 space-y-2">
                                    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 px-1">বিস্তারিত ঠিকানা</label>
                                    <textarea
                                        value={data.address}
                                        onChange={(e) => setData('address', e.target.value)}
                                        placeholder="বাসা নম্বর, রোড, এলাকা ও শহর"
                                        rows="4"
                                        className="w-full bg-white border border-slate-100 px-6 py-4 rounded-3xl text-sm font-bold focus:ring-2 focus:ring-emerald-100 transition-all shadow-sm resize-none"
                                    ></textarea>
                        ...
                        <section>
                            <div className="flex items-center gap-4 mb-8">
                                ...
                                <h2 className="text-2xl font-black text-slate-800 tracking-tight">ডেলিভারি পদ্ধতি</h2>
                            </div>
                                ...
                                {shipping_methods?.map((method) => (
                                    ...
                                    >
                                        ...
                                        <p className={cn(
                                            ...
                                            shippingMethodId === method.id ? "text-emerald-600" : "text-slate-800"
                                        )}>{method.name}</p>
                        ...
                        <section>
                            <div className="flex items-center gap-4 mb-8">
                                ...
                                <h2 className="text-2xl font-black text-slate-800 tracking-tight">পেমেন্ট পদ্ধতি</h2>
                            </div>
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className={cn(
                                                ...
                                                data.payment_method === gateway.code ? "bg-emerald-600" : "bg-slate-300"
                                            )}></div>
                                            <div className="relative z-10 flex items-center justify-between">
                                                ...
                                                <div className={cn(
                                                    ...
                                                    data.payment_method === gateway.code
                                                        ? "border-emerald-600 bg-white ring-4 ring-slate-800"
                                                        : "border-slate-200 bg-transparent group-hover:border-emerald-400"
                                                )}></div>
                                            </div>
                                        </button>
                                    );
                    ...
                    <div className="lg:col-span-5">
                        <div className="bg-white rounded-[4rem] p-10 border border-slate-100 shadow-2xl shadow-emerald-100/30 sticky top-32">
                            <h2 className="text-2xl font-black text-slate-800 tracking-tight mb-8">অর্ডার রিভিউ</h2>
                                ...
                                {cartItems.map((item) => (
                                    <div key={item.id} className="flex gap-4">
                                        <div className="w-16 h-16 bg-slate-50 rounded-2xl overflow-hidden shrink-0">
                                            <img src={item.image ? (item.image.startsWith('http') ? item.image : `/uploads/${item.image.replace(/^\//, '')}`) : 'https://images.unsplash.com/photo-1560343090-f0409e92791a?auto=format&fit=crop&q=80&w=800'} className="w-full h-full object-cover" />
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <p className="font-black text-slate-800 text-sm truncate">{item.name}</p>
                                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">পরিমাণ: {item.quantity} × ৳{parseFloat(item.price).toLocaleString()}</p>
                                        </div>
                            ...
                            <div className="space-y-4 pt-8 border-t border-slate-50 mb-10">
                                ...
                                <div className="flex justify-between items-center pt-4">
                                    <span className="text-xl font-black text-slate-800">সর্বমোট</span>
                                    <span className="text-4xl font-black text-emerald-600 tracking-tighter">৳{total.toLocaleString()}</span>
                                </div>
                            </div>

                            <button
                                disabled={processing}
                                className="w-full bg-emerald-600 hover:bg-emerald-700 text-white py-6 rounded-3xl font-black text-sm uppercase tracking-widest shadow-2xl shadow-emerald-200 transition-all flex items-center justify-center gap-3 disabled:opacity-50 group active:scale-[0.98]"
                            >
                                {processing ? (
                                    <Loader2 className="animate-spin" size={24} />
                                ) : (
                                    <>অর্ডার নিশ্চিত করুন <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" /></>
                                )}
                            </button>
                            ...
                            <div className="mt-10 p-6 bg-slate-50 rounded-[2rem] border border-slate-100 flex items-center gap-4">
                                ...
                                <p className="text-[10px] font-bold text-slate-500 uppercase leading-relaxed tracking-widest">
                                    অর্ডারটি কনফার্ম করার মাধ্যমে আপনি আমাদের শর্তাবলী এবং রিটার্ন পলিসির সাথে একমত পোষণ করছেন।
                                </p>
                            </div>
                        </div>
                    </div>
