...
export default function Index({ districts, filters }) {
    const [search, setSearch] = useState(filters.search || '');

    const handleSearch = (e) => {
        e.preventDefault();
        router.get(route('admin.districts.index'), { search }, { preserveState: true });
    };
        ...
        <AdminLayout>
            ...
            <div className="space-y-8 animate-in fade-in duration-500">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                        <h1 className="text-3xl font-black text-slate-800 tracking-tight mb-2">Regional Hubs</h1>
                        <p className="text-slate-500 font-medium">Manage districts to categorize products by their geographical origin.</p>
                    </div>
                    <Link
                ...
                <div className="bg-white rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden">
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-slate-50/50">
                                    <th className="px-10 py-6 text-[10px] font-black uppercase text-slate-400 tracking-widest">District Info</th>
                                    <th className="px-10 py-6 text-[10px] font-black uppercase text-slate-400 tracking-widest">Division</th>
                                    <th className="px-10 py-6 text-[10px] font-black uppercase text-slate-400 tracking-widest text-center">Order</th>
                                        ...
                                        <td className="px-10 py-6">
                                            <div className="flex items-center gap-4">
                                                <div className="w-14 h-14 bg-slate-100 rounded-2xl overflow-hidden shrink-0 border border-slate-50">
                                                    {district.image ? (
                                                        <img src={`/uploads/${district.image}`} className="w-full h-full object-cover" />
                                                    ) : (
                                                ...
                                                <div>
                                                    <p className="font-black text-slate-800 text-sm group-hover:text-emerald-600 transition-colors uppercase tracking-tight">{district.name}</p>
                                                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mt-0.5">Slug: {district.slug}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-10 py-6">
                                            <span className="text-xs font-bold text-slate-500 bg-slate-50 px-3 py-1.5 rounded-xl border border-slate-100">
                                                {district.division || 'N/A'}
                                            </span>
                                        </td>
                                        <td className="px-10 py-6 text-center">
                                            <span className="text-xs font-black text-slate-400">{district.sort_order}</span>
                                        </td>
                                        <td className="px-10 py-6">
                    ...
                    <div className="px-10 py-8 bg-slate-50/50 flex flex-col md:flex-row items-center justify-between gap-6 border-t border-slate-100">
                        <p className="text-xs font-bold text-slate-400">
                            Showing <span className="text-slate-800">{districts.from}</span> to <span className="text-slate-800">{districts.to}</span> of <span className="text-slate-800">{districts.total}</span> districts
                        </p>
