            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div className="flex p-1 bg-white rounded-2xl border border-slate-100 shadow-sm w-fit">
                        {['Daily', 'Weekly', 'Monthly', 'Yearly'].map((range) => (
                            <button
                                key={range}
                                className={`px-4 py-2 text-xs font-black uppercase tracking-widest rounded-xl transition-all ${range === 'Monthly' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-200' : 'text-slate-400 hover:text-slate-600'}`}
                            >
                                {range}
                            </button>
                        ))}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden group">
                        <div className="absolute top-0 right-0 w-32 h-32 bg-emerald-500/10 blur-[60px] rounded-full -mr-16 -mt-16 group-hover:scale-125 transition-transform duration-1000"></div>
                    ...
                    <div className="bg-emerald-600 rounded-[2.5rem] p-8 text-white relative overflow-hidden group shadow-xl shadow-emerald-100">
                        <div className="absolute inset-0 bg-gradient-to-tr from-emerald-700 to-emerald-600"></div>
                        <div className="relative z-10 flex flex-col h-full justify-between">
                            <div className="flex justify-between items-start">
                                <h4 className="text-[10px] font-black uppercase text-emerald-200 tracking-[0.3em]">Quick Strategic View</h4>
                                <BarChart3 size={18} className="text-emerald-200" />
                            </div>
                            <div className="mt-4">
                                ...
                                <Link href={route('admin.reports.index')} className="inline-block bg-white text-emerald-600 px-6 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest hover:shadow-lg transition-all">
                                    Open Report Hub
                                </Link>
                            </div>
                        </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100 flex flex-col h-[450px]">
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center">
                            <div className="w-full h-56 relative min-h-[224px]">
                                <ResponsiveContainer width="99%" height={224} debounce={1}>
                                    <PieChart>
                                        <Pie
                                            data={pieData}
                                            innerRadius={60}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-12">
                        ...
                        <div className="space-y-8 relative before:absolute before:inset-y-0 before:left-5 before:w-0.5 before:bg-slate-50">
                            {recentActivity.slice(0, 5).map((order, i) => (
                                <div key={order.id} className="flex gap-6 relative">
                                    <div className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 z-10 shadow-lg ${['bg-emerald-600', 'bg-fuchsia-600', 'bg-emerald-500', 'bg-amber-500', 'bg-emerald-600'][i % 5]
                                        } text-white`}>
                                        <Clock size={18} />
                                    </div>
                                    <div>
                    ...
                    <div className="lg:col-span-2 bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden">
                        <div className="flex items-center justify-between mb-8">
                            ...
                            <Link href={route('admin.orders.index')} className="text-xs font-black text-emerald-600 uppercase tracking-widest hover:underline px-4 py-2 bg-emerald-50 rounded-xl">View All</Link>
                        </div>
