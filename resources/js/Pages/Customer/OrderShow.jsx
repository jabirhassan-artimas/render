            ...
            <div className="space-y-10">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="flex items-center gap-6">
                        <Link href={route('customer.orders')} className="w-12 h-12 bg-white border border-slate-100 rounded-2xl flex items-center justify-center text-slate-400 hover:text-blue-600 hover:border-blue-100 transition-all shadow-sm group">
                    ...
                    <div className="flex items-center gap-3">
                        <button className="bg-white border border-slate-100 text-slate-400 hover:text-slate-800 px-6 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center gap-2 transition-all shadow-sm">
                            ...
                            {statusSteps.map((step, i) => (
                                <div key={step} className="flex-1 min-w-[120px] flex flex-col items-center group">
                                    <div className={cn(
                                        ...
                                        i <= currentStepIndex ? "bg-blue-600 text-white scale-110 shadow-xl shadow-blue-200" : "bg-slate-50 text-slate-300"
                                    )}>
                                        {i === 0 && <Package size={20} />}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                        ...
                        <section className="bg-white rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden">
                            <div className="p-8 border-b border-slate-50">
                                <h3 className="text-xl font-black text-slate-800 tracking-tight flex items-center gap-3">
                                    <ShoppingBag size={20} className="text-blue-600" /> Items Summary
                                </h3>
                            </div>
                                ...
                                {order.items.map((item) => (
                                    <div key={item.id} className="p-8 flex items-center gap-6 group hover:bg-slate-50/30 transition-colors">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <h4 className="font-black text-slate-800 tracking-tight group-hover:text-blue-600 transition-colors truncate">{item.product_name}</h4>
                    ...
                    <div className="lg:col-span-4 space-y-8">
                        <div className="bg-slate-900 rounded-[3.5rem] p-10 text-white shadow-3xl shadow-slate-200 relative overflow-hidden">
                            <div className="absolute top-0 right-0 w-32 h-32 bg-blue-600 opacity-20 blur-2xl rounded-full -mr-16 -mt-16"></div>
                            <h3 className="text-xl font-black mb-10 tracking-tight flex items-center gap-3">
                                <Truck size={20} className="text-blue-500" /> Delivery Details
                            </h3>
                            ...
                            <div className="space-y-8">
                                <div className="flex gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm font-bold text-blue-100">{order.user?.name || 'Valued Customer'}</p>
                                    </div>
                                </div>
                                <div className="flex gap-4">
                                    <div className="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center text-white/40 shrink-0">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm font-bold text-blue-100">{order.phone}</p>
                                    </div>
                                </div>
                                <div className="flex gap-4">
                                    <div className="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center text-white/40 shrink-0">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm font-bold text-blue-100 leading-relaxed">{order.shipping_address}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-[3.5rem] p-10 border border-slate-100 shadow-sm">
                            ...
                            <div className="space-y-4 mb-8">
                                ...
                                <div className="flex justify-between items-baseline pt-2">
                                    ...
                                    <span className="text-3xl font-black text-blue-600 tracking-tighter">৳{parseFloat(order.total).toLocaleString()}</span>
                                </div>
                            </div>
                            <div className="p-4 bg-slate-50 rounded-2xl flex items-center justify-between">
                                <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Payment</span>
                                <span className="text-[10px] font-black uppercase tracking-widest text-blue-600">{order.payment_method}</span>
                            </div>
                        </div>
                    </div>
                </div>
